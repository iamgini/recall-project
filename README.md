...
Save this key — you'll use it for local dev, Cloudflare, CLI, and the browser UI.

### 2. Run locally with Podman

```bash
cd ~/workarea/recall-project
RECALL_API_KEY=<your-key> ./dev.sh
```

Open http://localhost:8788

### 3. Set the key in the browser

Click **Settings** in the toolbar → paste your API key → Save.
...
`./dev.sh` handles everything: builds the image, creates a Podman volume for data, runs the app.

```bash
# Start (API key is required for add/edit/delete)
RECALL_API_KEY=<your-key> ./dev.sh
...
Bookmarks persist in a Podman volume (`recall-data`). Ctrl+C to stop.
...
| Environment | How |
|-------------|-----|
| Podman (local) | `RECALL_API_KEY=<key> ./dev.sh` |
